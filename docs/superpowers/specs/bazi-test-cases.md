...
**lunar-javascript 实测排盘（P1 baseline，待用户用权威 App 校对）**：

- 真太阳时偏差：约 +0.62 分钟（杭州 lng 120.16，离 120 仅差 0.16°）
- 四柱：庚午年 / 壬午月 / **辛亥**日 / 乙未时
- 日主：辛（金）
- 大运起运：1 岁 → 8 岁起首步大运（癸未/甲申/乙酉/丙戌/丁亥/戊子/己丑/庚寅，共 8 步）
- 校对来源：lunar-javascript v1.7.7 直接输出，**待权威 App 二次校对**

> 注：原计划猜测的"戊辰日"经 lunar-javascript 实测为错误，正确为辛亥日。这正是为何 P1 不依赖外部猜测的 ground truth，先以库内自洽建立 baseline，待用户用权威 App 校对后再升级为强 assertion。
