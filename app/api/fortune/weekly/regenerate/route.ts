...
import { profiles, fortunesWeekly } from "@/lib/db/schema";
import { fetchWeeklyFortune } from "@/lib/fortune/fetch-weekly";
...
/**
 * POST /api/fortune/weekly/regenerate — 把 weekly fallback reading 升级到 AI 个性化版
 *
 * 与 /api/fortune/today/regenerate 同模式（SWR），仅作用于 fortunes_weekly。
...
export async function POST(req: Request): Promise<Response> {
  ...
  let fortune;
  try {
    fortune = fetchWeeklyFortune({ userId, anchorDate: parsed.data.anchorDate });
  } catch (e) {
  ...
  const prompt = buildPeriodReadingPrompt({
    period: "week",
    anchorDate: fortune.weekStart,
    rangeHint: fortune.rangeHint,
  ...
  try {
    const result = await chat({
      ...
      thinking: "disabled",
      meta: { conversationId: "fortune-reading-weekly", userId },
    });
    aiText = result.text;
  } catch {
    return NextResponse.json(
  ...
  try {
    db.update(fortunesWeekly)
      ...
      .where(
        and(
          eq(fortunesWeekly.profile_id, fortune.profileId),
          eq(fortunesWeekly.week_start, fortune.weekStart),
        ),
      )
      .run();
  } catch (e) {
    console.error("[regenerate-weekly] update fortunes_weekly 失败", e);
    return NextResponse.json(
