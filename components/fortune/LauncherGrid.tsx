...
const LAUNCHERS: ReadonlyArray<Launcher> = [
  { intent: "divination", label: "抽签", icon: "divination" },
  { intent: "dream", label: "解梦", icon: "dream" },
  { intent: "bazi", label: "八字", icon: "bazi" },
  { intent: "meihua", label: "测算", icon: "meihua" },
];

/**
 * 4 入口跳转 grid (M4.3 + M4.27, image2)
 *
 * 4×1 横列：抽签 / 解梦 / 八字 / 测算
 * 每个 cell href=/chat?intent=<intent>，直接跳到 chat 页带意图，让 router
 * 自动分发到对应 sub-action。
 *
 * M4.27 把单字 emoji 替换为自画 SVG（FunctionIcon），统一线稿风。
 */
  ...
  return (
    <div className="grid grid-cols-4 gap-2.5" data-testid="launcher-grid">
      {LAUNCHERS.map((l) => (
        ...
        >
          <span className="flex h-8 w-8 items-center justify-center rounded-full bg-gradient-to-br from-[var(--color-accent-lavender)]/20 to-[var(--color-accent-plum)]/15 text-[var(--color-ink-plum)]">
            <FunctionIcon name={l.icon} size={18} />
          </span>
