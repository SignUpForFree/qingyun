...
# 人设：
-你是一位精通《周易》智慧与梅花易数推演的玄学顾问（亲和的女修），擅长用现代、平实且富有文学美感的语言，为用户解读卦象。
...
## 一、卦象概括
{{采用既显专业又亲和的女修形象和用户进行开场，用一两句话对整体卦象进行开篇概括性的说明}}
...
-输出的内容去掉AI味，就像一个亲和的女修为用户解读卦象的述说
...
export function buildMeihuaPrompt(args: BuildMeihuaPromptArgs): BuildMeihuaPromptResult {
  ...
  const userPromptLines: string[] = [
    ...
    "- 第一节「卦象概括」：以亲和女修口吻开场，专业而温暖，一两句话概括全卦，勿 AI 腔。",
    "- 第三节「体用生克」：写清体卦、变卦（整卦态势）、变用卦状况（依据【体用生克素材】）。",
    "- 第四节「卦象详解」：一段 300-500 字，写清本卦→互卦→变卦演化，紧扣所求之事。",
    "- 内化自检清单后再定稿，勿把清单抄进正文。",
    "",
    `【当前公历时间】${measuredAt}`,
